...
class HomeInfoCard extends StatelessWidget {
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _buildInfoRow(icon: Icons.location_pin, text: location),
                          SizedBox(height: height * 0.01),
                          _buildInfoRow(icon: Icons.shopping_cart, text: quantity),
                          SizedBox(height: height * 0.01),
                          _buildInfoRow(imagePath: ImageAssets.crops, text:  item),
                        ],
                      ),
                    ),
                    if (cardType == CardType.transportation) const SizedBox(width: 16),
                    if (cardType == CardType.transportation && driverName != null)
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            if (cardType == CardType.transportation && vehicleNumber != null)
                              _buildInfoRow(imagePath: ImageAssets.truck, text: vehicleNumber!),
                            SizedBox(height: height * 0.01),
                            _buildInfoRow(icon: Icons.person, text: driverName!),
                           ],
                        ),
                      ),
                  ],
  ...
  // Info row helper
  Widget _buildInfoRow({IconData? icon, String? imagePath, required String text}) {
    return Row(
      children: [
        if (icon != null)
